...
import {ShowTareaRequest} from "../validators/TareasValidators";

const router = express.Router();
const notaController = new NotaController();

router.get("/prueba", notaController.pruebaNotaController);
router.post("/create_note/task/:id", middlewareJWT, CreateNoteRequest, ShowTareaRequest, notaController.create_note);
router.get("/find_note/:id", middlewareJWT, FindNoteRequest, notaController.find_note);
router.get("/find_all_notes/task/:id", middlewareJWT, ShowTareaRequest, notaController.find_all_notes);
