...
import type {ShowUserInfer} from "../types";
import {logoutUserGET} from "../services/UsersService.ts";
...
export default function NavMenu({data}: NavMenuProps) {
    ...
    async function logoutFunction() {
        const resultLogout = await logoutUserGET();
        if (resultLogout?.status) {
            navigate('/login');
        }
    }
            ...
            >
                <Popover.Panel
                    ...
                    <div
                        ...
                        <p className='text-center'>Hola: {data?.user.nombre}</p>
                        <Link
                            to='/profile'
